...
TODO: Properly integrate with kedro context/datasets rather than just using it to retrieve the file list.
"""
import hashlib
import os
import pathlib
...
class Dataset(AbstractDataSet):
    ...
    @classmethod
    def get_files(cls, context, stage: str = DEFAULT_STAGE):
        return getattr(context.catalog.datasets, f'{cls.NAME.lower()}_{stage}')._files
        ...
        if local:
            ...
            if cache:
                ...
                df['local_filepath'] = df.apply(
                    lambda x: cls.save_locally(check_md5=check_md5, ignore_errors=True, verbose=verbose, **x.to_dict()), axis=1)
                if load_audio:
